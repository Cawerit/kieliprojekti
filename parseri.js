...
function parse(tokenit, natiiviKutsut) {
      ...
      if (token.tyyppi === tokenTyypit.SULKU) {
        if (token.arvo === '(') {
          switch (edellinen && edellinen.tyyppi) {
            ...
            case parseriTyypit.NATIIVIKUTSU: // eslint-disable-line no-fallthrough
    ...
    function parseArgumenttiTaiParametriLista() {
      ...
      ulompi:
      while(indeksi + 1 < tokenit.length) {
        let ilmaisunOsat = [];
        do {
          seuraava();
          if (token.arvo === ')') {
        ...
        } while (indeksi + 1 < tokenit.length && tokenit[indeksi+1].tyyppi !== tokenTyypit.PILKKU);

        tulos.ilmaisut.push(ilmaisunOsat);
        console.log('token ensin', token);
        seuraava();
        console.log('token nyt', token);
      }
